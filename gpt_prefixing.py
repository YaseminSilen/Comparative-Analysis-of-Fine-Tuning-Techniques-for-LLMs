...
training_args = TrainingArguments(
    output_dir="./gpt2_finance_sentiment_prefix",
...
# 10. 保存模型
trainer.save_model("./gpt2_finance_sentiment_prefix")
...
config = PeftConfig.from_pretrained("./gpt2_finance_sentiment_prefix")
model = GPT2ForSequenceClassification.from_pretrained(config.base_model_name_or_path, num_labels=3)
model = PeftModel.from_pretrained(model, "./gpt2_finance_sentiment_prefix")
model.to(device)
...
final_metrics = compute_metrics((val_predictions.predictions, val_predictions.label_ids))

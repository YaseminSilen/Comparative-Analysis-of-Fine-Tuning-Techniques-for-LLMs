...
training_args = TrainingArguments(
    output_dir="./gpt2_finance_sentiment_lora",
...
# 7. 保存模型
trainer.model.save_pretrained("./gpt2_finance_sentiment_lora")
...
class_weights = class_weights.to(device)
# 加载配置
config = PeftConfig.from_pretrained("./gpt2_finance_sentiment_lora")

# 加载基础模型
model = GPT2ForSequenceClassification.from_pretrained(config.base_model_name_or_path, num_labels=3)

# 加载LoRA权重
model = PeftModel.from_pretrained(model, "./gpt2_finance_sentiment_lora")
model.to(device)
